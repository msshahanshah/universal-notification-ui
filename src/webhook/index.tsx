...
export default function WebhookConfigPage() {
  ...
  const {
    ...
    error: webhookError,
    isError,
  } = useListWebhookConfigs(clientId);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = editingWebhookId
        ? await updateMutation.mutateAsync({ ...payload, clientId: editingWebhookId } as any)
    ...
    <div style={{ maxWidth: 800, margin: "40px auto", padding: 20 }}>
      ...
      {webhooksLoading ? (
        ...
        >
          ...
          {(Array.isArray(webhooksList?.data?.data) ? webhooksList.data.data : []).map((webhook: any) => (
              <WebhookListItem
                key={webhook.id}
                webhook={webhook}
